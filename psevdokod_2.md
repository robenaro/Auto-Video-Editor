...
3.1 
    3.1.1 Цикл: Получить доступ к таймкодам массива arrayTiser с пмощью иддекса [0 + 4*n]; пока n != количеству всех элементов массива/4; после выполнение подпунктов n+1.
...
Слайд TransitionBlock имеет свойства:
    Заголовок
    текст (дополнительный параметр. Может быть пустым)

Проблема: слайдам нельзя вставлять жирность. Нужно пробовать как-то усовершенствовать имитацию жирности, чтобы это можно было делать автоматом. Или делать слайды в самом АЕ и там писать скрипты.
